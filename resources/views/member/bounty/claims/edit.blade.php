        ...
        <div class="row">
            <div class="col-6 pull-left text-left">
                <h3 class="title-wd" style="border-bottom: none;"><i class="fa fa-btc"></i>
                    Update Claim Bounty #{{ $bounty->id }} ({{ $bounty->name }})
                </h3>
            </div>
            ...
            <tbody>
                <tr>
                    <td class="text-center" style="padding: 20px;">
                        ...
                        <form name="claim" class="claim form" method="POST" action="{{ route('member.bounty.claim.edit.save', $claim->id) }}">
                            @csrf
                            <div class="row">
                                <div class="col-lg-12 no-pdd">
                                    <div class="sn-field input-group">
                                        <textarea rows="15" placeholder="Enter all information noted in the Bounty Description in order to receive credit for this claim!" id="description" type="description" class="form-control{{ $errors->has('description') ? ' is-invalid' : '' }}" name="description" required>{{ $claim->description }}</textarea>
                                    </div>
                                ...
                                <div class="col-lg-12 no-pdd text-right">
                                    ...
                                    @if ($bounty->isClaimable())
                                        <button type="submit" class="btn btn-primary btn-md">
                                            <i class="fa fa-plus-circle"></i> {{ __('Update Claim') }}
                                        </button>
                                    @else
